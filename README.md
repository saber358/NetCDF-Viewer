...
基于 JavaFX 的 NetCDF 桌面可视化工具。面向海洋、水文、环境和数值模拟数据分析场景，支持非结构化三角网与规则格网 `.nc` 数据读取、二维平面渲染、单点查询、多数据集管理、XYZ 在线底图、海岸线叠加、波浪叠加、海流流线叠加、风场风羽叠加与 PNG 导出。
...
- 支持平面图单点查询
- 支持 XYZ 在线底图，可使用 OpenStreetMap 标准地图或自定义瓦片 URL 模板
...
- 打开数据后默认不显示海岸线，可通过菜单手动开启内置海岸线
- 底图图层固定绘制在最下层，支持透明度调节和本地瓦片缓存
- 主界面、状态栏、对话框和关于信息已切换为中文文案
- 缩放和平移时先用缓存画面预览，停止交互后再精细重绘，减少拖拽卡顿
- 三角网查询引入空间索引，波浪、海流、风场和单点查询共用同一套候选三角形加速链路
- 渲染主链改为固定线程池调度，规则格网底图支持并行像素缓冲栅格化
- 多数据集切换后会同步刷新变量列表、查询上下文和叠加状态，并可通过勾选框控制哪些数据参与画布渲染
...
OpenStreetMap 标准底图与 NetCDF 标量场叠加，底图绘制在最下层，海岸线和数据范围保持同一经纬度视口。

![在线底图叠加](docs/images/readme/preview-basemap-rendering.png)
...
6. 如果数据坐标为经纬度，可在右侧启用底图并选择 OpenStreetMap 或自定义 XYZ 模板
7. 点击画布执行单点查询
8. 点击 `Export PNG` 导出当前视图

## 底图说明

第一版底图支持 XYZ 瓦片 URL 模板，模板必须包含 `{z}`、`{x}`、`{y}`。例如：

```text
https://tile.openstreetmap.org/{z}/{x}/{y}.png
```

底图只会在数据坐标范围符合经纬度边界时绘制。投影坐标或本地平面坐标不会强行叠加在线底图，避免位置误导。

如果文件坐标变量看起来是经纬度，但解析出的 X/Y 范围不符合经纬度边界，程序会先提示风险，由用户决定是否继续加载。
