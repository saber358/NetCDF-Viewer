...
一款基于 JavaFX 的 NetCDF 非结构化三角网桌面可视化工具，支持读取包含节点坐标、三角形连接关系和属性变量的 `.nc` 文件，并进行二维平面填色、单点查询、多数据集切换、海岸线叠加、波向箭头叠加、海流流线叠加与 PNG 导出。
...
- 自动识别坐标变量、连接关系、可平面化属性变量、波浪变量对和流场速度变量对
- 同时支持节点中心与单元中心属性变量显示
- 支持多层变量深度切换、平面图单点查询与多数据集管理
- 支持 GeoJSON / shapefile 海岸线叠加，并内置默认海岸线资源
- 支持波向箭头与海流动态流线叠加，适合海洋流场和波浪场分析
...
- 支持单层变量与多层变量的平面渲染
- 支持深度层切换
- 支持平面图单点查询，节点中心变量返回插值值，单元中心变量返回命中单元原始值
- 支持颜色映射与当前层最小值 / 最大值显示
- 支持 GeoJSON / shapefile 海岸线叠加与内置海岸线恢复
- 支持自动识别 `wdir` / `wlen` 波场变量并叠加波向箭头
- 支持自动识别 `u` / `v` 或 `ua` / `va` 流场变量并叠加动态海流流线
- 支持导出当前可视化结果为 PNG 图片
